...
template <typename PointSourceType, typename PointTargetType>
Eigen::Matrix3d NormalDistributionsTransform<PointSourceType, PointTargetType>::getCovariance()
{
  return intersection_cov;
}

template <typename PointSourceType, typename PointTargetType>
Eigen::Matrix<double,6,1> NormalDistributionsTransform<PointSourceType, PointTargetType>::getPdelta()
{
  return delta_p;
}

template <typename PointSourceType, typename PointTargetType>
Eigen::Matrix<double,6,6> NormalDistributionsTransform<PointSourceType, PointTargetType>::getHessian()
{
...
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computeTransformation(const Eigen::Matrix<float, 4, 4> &guess)
{
  ...
  Eigen::Matrix<double, 6, 1> p, score_gradient;
  ...
  if (source_cloud_->points.size() > 0) {
    std::cout<<"point_size: "<<source_cloud_->points.size()<<std::endl;
...
// template <typename PointSourceType, typename PointTargetType>
// double NormalDistributionsTransform<PointSourceType, PointTargetType>::getCovariance(Eigen::Matrix<double, 6, 1> pose)
                                              ...
                                              Eigen::Matrix<double, 6, 1> pose, bool compute_hessian)
{
  ...
  Eigen::Vector3d x, x_trans, prev_mean, intersection_mean, mean;

  Eigen::Matrix3d c_inv ,prev_c_inv;
  ...
  // int* covArray= new int[source_cloud_->points.size()]
  for (int idx = 0; idx < source_cloud_->points.size(); idx++) {
    ...
    for (int i = 0; i < neighbor_ids.size(); i++) {
      ...
      mean = voxel_grid_.getCentroid(vid);
      c_inv = voxel_grid_.getInverseCovariance(vid);
      // std::cout<<"covariance_inverse"<<std::endl;
      // std::cout<<c_inv<<std::endl;
      // if(i==0 & idx==0){
      //   voxel_grid_.initial_cov(c_inv,mean);
      // }
      // else{
      //   voxel_grid_.fuse(c_inv, mean);
  ...
  // intersection_cov = voxel_grid_.getCovariance();
...
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::computeUncertainLayer(const Eigen::Matrix<float, 4, 4> &guess)
{
  if (guess != Eigen::Matrix4f::Identity()) {
    final_transformation_ = guess;
  ...
  for (int idx = 0; idx < source_cloud_->points.size(); idx++) {
    neighbor_ids.clear();
    x_trans_pt = trans_cloud_.points[idx];
    ...
    for (int i = 0; i < neighbor_ids.size(); i++) {
      ...
      variance += x_trans.dot(c_inv * x_trans);
    }
  }
