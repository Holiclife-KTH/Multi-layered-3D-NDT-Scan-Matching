...
static Eigen::Matrix<double, 6,6> hessian_inv;
static Eigen::Matrix<double, 6,6> cov;
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    else if (_method_type == MethodType::PCL_ANH)
    {
      ...
      hessian_inv = anh_ndt.getHessian();
      cov = fitness_score * hessian_inv;
    }
#ifdef CUDA_FOUND
    else if (_method_type == MethodType::PCL_ANH_GPU)
    {
    ...
    else
    {
      ...
      ndt_pose_msg.pose.position.z = -0.49;
    ...
    */
    ...
    std::cout << "-----------------------------------------------------------------" << std::endl;
    std::cout << "Sequence222: " << input->header.seq << std::endl;
    std::cout << "Timestamp222: " << input->header.stamp << std::endl;
    std::cout << "Frame ID222: " << input->header.frame_id << std::endl;
    //    std::cout << "Number of Scan Points: " << scan_ptr->size() << " points." << std::endl;
    std::cout << "Number of Filtered Scan Points222: " << scan_points_num << " points." << std::endl;
    std::cout << "NDT has converged222: " << has_converged << std::endl;
    std::cout << "Fitness Score222: " << fitness_score << std::endl;
    std::cout << "Transformation Probability222: " << trans_probability << std::endl;
    std::cout << "Execution Time222: " << exe_time << " ms." << std::endl;
    std::cout << "Number of Iterations222: " << iteration << std::endl;
    std::cout << "NDT Reliability2: " << ndt_reliability.data << std::endl;
    std::cout << "(x,y,z,roll,pitch,yaw)222: " << std::endl;
    std::cout << "(" << current_pose.x << ", " << current_pose.y << ", " << current_pose.z << ", " << current_pose.roll
              << ", " << current_pose.pitch << ", " << current_pose.yaw << ")" << std::endl;
    std::cout << "Transformation Matrix222: " << std::endl;
    std::cout << t << std::endl;
    std::cout << "Align time222: " << align_time << std::endl;
    std::cout << "Get fitness score time222: " << getFitnessScore_time << std::endl;
    std::cout << "-----------------------------------------------------------------" << std::endl;
...
void* thread_func(void* args)
{
  ...
  ros::Subscriber map_sub = nh_map.subscribe("mid_points_map", 10, map_callback);
...
{
  ros::init(argc, argv, "ndt_matching2");
  ...
  try
  {
    tf_baselink2primarylidar =
      tf_buffer.lookupTransform("chassis_link", lidar_frame, ros::Time().now(), ros::Duration(3.0));
  }
  catch (tf2::TransformException& ex)
  {
  ...
  // Publishers
  predict_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose2", 10);
  predict_pose_imu_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_imu2", 10);
  predict_pose_odom_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_odom2", 10);
  predict_pose_imu_odom_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_imu_odom2", 10);
  ndt_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/ndt_pose2", 10);
  // current_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/current_pose", 10);
  localizer_pose_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/localizer_pose2", 10);
  estimate_twist_pub = nh.advertise<geometry_msgs::TwistStamped>("/estimate_twist2", 10);
  estimated_vel_mps_pub = nh.advertise<std_msgs::Float32>("/estimated_vel_mps2", 10);
  estimated_vel_kmph_pub = nh.advertise<std_msgs::Float32>("/estimated_vel_kmph2", 10);
  estimated_vel_pub = nh.advertise<geometry_msgs::Vector3Stamped>("/estimated_vel2", 10);
  time_ndt_matching_pub = nh.advertise<std_msgs::Float32>("/time_ndt_matching2", 10);
  ndt_stat_pub = nh.advertise<autoware_msgs::NDTStat>("/ndt_stat2", 10);
  ndt_reliability_pub = nh.advertise<std_msgs::Float32>("/ndt_reliability2", 10);
  ...
  ros::Subscriber points_sub = nh.subscribe("filtered_mid_points", _queue_size, points_callback);
