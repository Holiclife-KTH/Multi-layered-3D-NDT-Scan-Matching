...
static geometry_msgs::PoseWithCovarianceStamped localizer_pose_msg;

static ros::Publisher estimate_twist_pub;
static geometry_msgs::TwistStamped estimate_twist_msg;
static ros::Publisher ndt_heading_cov_pub;
static std_msgs::Float32 heading_yaw;
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    else if (_method_type == MethodType::PCL_ANH)
    {
      ...
      cov = anh_ndt.getCovariance();
      deviation = anh_ndt.getPdelta();
    }
#ifdef CUDA_FOUND
    else if (_method_type == MethodType::PCL_ANH_GPU)
    {
    ...
    */
    ...
    localizer_q.setRPY(0.0, 0.0, localizer_pose.yaw);
    if (_use_local_transform == true)
    {
      ...
      localizer_pose_msg.header.stamp = current_scan_time;
        localizer_pose_msg.pose.pose.position.x = (local_transform * transform).getOrigin().getX();
      localizer_pose_msg.pose.pose.position.y = (local_transform * transform).getOrigin().getY();
      localizer_pose_msg.pose.pose.position.z = (local_transform * transform).getOrigin().getZ();
      localizer_pose_msg.pose.pose.orientation.x = (local_transform * transform).getRotation().x();
      localizer_pose_msg.pose.pose.orientation.y = (local_transform * transform).getRotation().y();
      localizer_pose_msg.pose.pose.orientation.z = (local_transform * transform).getRotation().z();
      localizer_pose_msg.pose.pose.orientation.w = (local_transform * transform).getRotation().w();
    }
    else
    {
      localizer_pose_msg.header.frame_id = "map";
      localizer_pose_msg.header.stamp = current_scan_time;
      localizer_pose_msg.pose.pose.position.x = localizer_pose.x;
      localizer_pose_msg.pose.pose.position.y = localizer_pose.y;
      // localizer_pose_msg.pose.pose.position.z = localizer_pose.z;
      localizer_pose_msg.pose.pose.position.z = -0.49;
      localizer_pose_msg.pose.pose.orientation.x = localizer_q.x();
      localizer_pose_msg.pose.pose.orientation.y = localizer_q.y();
      localizer_pose_msg.pose.pose.orientation.z = localizer_q.z();
      localizer_pose_msg.pose.pose.orientation.w = localizer_q.w();
      localizer_pose_msg.pose.covariance[0] = cov(0,0);
      localizer_pose_msg.pose.covariance[1] = cov(0,1);
      localizer_pose_msg.pose.covariance[2] = cov(0,2);
      localizer_pose_msg.pose.covariance[6] = cov(1,0);
      localizer_pose_msg.pose.covariance[7] = cov(1,1);
      localizer_pose_msg.pose.covariance[8] = cov(1,2);
      localizer_pose_msg.pose.covariance[12] = cov(2,0);
      localizer_pose_msg.pose.covariance[13] = cov(2,1);
      localizer_pose_msg.pose.covariance[14] = cov(2,2);
    ...
    heading_yaw.data = deviation(6,1);
    ndt_heading_cov_pub.publish(heading_yaw);
    ...
    ndt_reliability.data = Wa * (exe_time / 100.0) * 100.0 + Wb * (iteration / 10.0) * 100.0 +
                           Wc * (-std::fabs(2.0 - trans_probability) / 2.0) * 100.0;
    ...
    std::cout << "-----------------------------------------------------------------" << std::endl;
    std::cout << "Sequence333: " << input->header.seq << std::endl;
    std::cout << "Timestamp333: " << input->header.stamp << std::endl;
    std::cout << "Frame ID333: " << input->header.frame_id << std::endl;
    //    std::cout << "Number of Scan Points: " << scan_ptr->size() << " points." << std::endl;
    std::cout << "Number of Filtered Scan Points333: " << scan_points_num << " points." << std::endl;
    std::cout << "NDT has converged333: " << has_converged << std::endl;
    std::cout << "Fitness Score333: " << fitness_score << std::endl;
    std::cout << "Transformation Probability333: " << trans_probability << std::endl;
    std::cout << "Execution Time333: " << exe_time << " ms." << std::endl;
    std::cout << "Number of Iterations333: " << iteration << std::endl;
    std::cout << "NDT Reliability333: " << ndt_reliability.data << std::endl;
    std::cout << "(x,y,z,roll,pitch,yaw)333: " << std::endl;
    std::cout << "(" << current_pose.x << ", " << current_pose.y << ", " << current_pose.z << ", " << current_pose.roll
              << ", " << current_pose.pitch << ", " << current_pose.yaw << ")" << std::endl;
    std::cout << "Transformation Matrix333: " << std::endl;
    std::cout << t << std::endl;
    std::cout << "Align time333: " << align_time << std::endl;
    std::cout << "Get fitness score time333: " << getFitnessScore_time << std::endl;
    std::cout << "-----------------------------------------------------------------" << std::endl;
...
void* thread_func(void* args)
{
  ...
  ros::Subscriber map_sub = nh_map.subscribe("high_points_map", 10, map_callback);
...
{
  ros::init(argc, argv, "ndt_matching3");
  ...
  try
  {
    tf_baselink2primarylidar =
      tf_buffer.lookupTransform("chassis_link", lidar_frame, ros::Time(0), ros::Duration(3.0));
  }
  catch (tf2::TransformException& ex)
  {
  ...
  // Publishers
  predict_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose3", 10);
  predict_pose_imu_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_imu3", 10);
  predict_pose_odom_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_odom3", 10);
  predict_pose_imu_odom_pub = nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_imu_odom3", 10);
  ndt_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/ndt_pose3", 10);
  // current_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/current_pose", 10);
  localizer_pose_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/localizer_pose3", 10);
  estimate_twist_pub = nh.advertise<geometry_msgs::TwistStamped>("/estimate_twist3", 10);
  estimated_vel_mps_pub = nh.advertise<std_msgs::Float32>("/estimated_vel_mps3", 10);
  estimated_vel_kmph_pub = nh.advertise<std_msgs::Float32>("/estimated_vel_kmph3", 10);
  estimated_vel_pub = nh.advertise<geometry_msgs::Vector3Stamped>("/estimated_vel3", 10);
  time_ndt_matching_pub = nh.advertise<std_msgs::Float32>("/time_ndt_matching3", 10);
  ndt_stat_pub = nh.advertise<autoware_msgs::NDTStat>("/ndt_stat3", 10);
  ndt_reliability_pub = nh.advertise<std_msgs::Float32>("/ndt_reliability3", 10);
  ndt_heading_cov_pub = nh.advertise<std_msgs::Float32>("/heading_cov3", 1);
  ...
  ros::Subscriber points_sub = nh.subscribe("filtered_high_points", _queue_size, points_callback);
