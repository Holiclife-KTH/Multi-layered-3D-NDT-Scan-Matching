...
template <typename PointSourceType>
Eigen::Matrix3d VoxelGrid<PointSourceType>::getCovariance() const
{
  return cov;
}

template <typename PointSourceType>
Eigen::Vector3d VoxelGrid<PointSourceType>::getMean() const
{
  return mean;
}
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::initial_cov(Eigen::Matrix3d &inv_c, Eigen::Vector3d &Mean){

prev_cov = inv_c.inverse();
prev_mean= Mean;
}
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::fuse(Eigen::Matrix3d &inv_c, Eigen::Vector3d &Mean){
float omega = 0;
...
omega = optimize(prev_cov, inv_c);
cov = (prev_cov.inverse() + inv_c - (((prev_cov)*omega + (inv_c.inverse())*(1-omega)).inverse())).inverse();
T = (prev_cov*omega + inv_c.inverse()*(1-omega)).inverse();
K = (cov*(prev_cov.inverse()-omega*T));
L = (cov*(inv_c-(1-omega)*T));
mean= K*prev_mean + L*Mean;
...
template <typename PointSourceType>
float VoxelGrid<PointSourceType>::optimize(Eigen::Matrix3d &c_a, Eigen::Matrix3d &inv_c_b){
  float omega = 0.0;
  float new_omega = 0;
  float prev_omega = 1.0;
  Eigen::Matrix3d inv_c_a;
  Eigen::Matrix3d c_b;
  inv_c_a = c_a.inverse();
  c_b = inv_c_b.inverse();
  for (int i=1; i<=10; i++){
    omega = i * 0.1;
    new_omega = (((inv_c_a+inv_c_b)-((c_a*omega+c_b*(1-omega)).inverse())).inverse()).trace();
    if (prev_omega > new_omega){
      prev_omega = new_omega;
    }
    
  }
  // std::cout<<"omege: "<<prev_omega<<std::endl;
  return prev_omega;
}
