...
void publish_pcd(sensor_msgs::PointCloud2 pcd, const int* errp = NULL)
{
  if (pcd.width != 0)
  {
    ...
    for (int point_id=0; point_id < low_layered_map->points.size(); ++point_id) {
        if(low_layered_map->points[point_id].z < height){
...
int main(int argc, char** argv)
{
  ...
  AR_pcd_pub = nh.advertise<sensor_msgs::PointCloud2>("low_points_map", 1, true);
  ...
  height_sub = nh.subscribe("object_height", 10, height_callback);
  if (margin < 0)
  {
    int err = 0;
