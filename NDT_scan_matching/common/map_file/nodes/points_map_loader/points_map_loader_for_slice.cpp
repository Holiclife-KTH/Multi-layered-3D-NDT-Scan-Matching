...
pcl::PCLPointCloud2 low_cloud;
pcl::PCLPointCloud2 mid_cloud;
pcl::PCLPointCloud2 high_cloud;
...
ros::Publisher low_pcd_pub;
ros::Publisher mid_pcd_pub;
ros::Publisher high_pcd_pub;
...
void publish_pcd(sensor_msgs::PointCloud2 pcd, const int* errp = NULL)
{
  if (pcd.width != 0)
  {
    pcl::PointCloud<pcl::PointXYZI>::Ptr low_layered_map(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr mid_layered_map(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr high_layered_map(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::fromROSMsg(pcd, *low_layered_map);
    pcl::fromROSMsg(pcd, *mid_layered_map);
    pcl::fromROSMsg(pcd, *high_layered_map);
    for (int point_id=0; point_id < low_layered_map->points.size(); ++point_id) {
        if(low_layered_map->points[point_id].z >=0.21 || low_layered_map->points[point_id].z < -0.49){
            low_layered_map->points[point_id].x=0;
            low_layered_map->points[point_id].y=0;
            low_layered_map->points[point_id].z=0;
            low_layered_map->points[point_id].intensity=1;
        }

        if(mid_layered_map->points[point_id].z >= 0.81 || mid_layered_map->points[point_id].z < 0.21){
            mid_layered_map->points[point_id].x=0;
            mid_layered_map->points[point_id].y=0;
            mid_layered_map->points[point_id].z=0;
            mid_layered_map->points[point_id].intensity=2;
        }

        if(high_layered_map->points[point_id].z < 0.81 || high_layered_map->points[point_id].z >= 3.66 ){
            high_layered_map->points[point_id].x=0;
            high_layered_map->points[point_id].y=0;
            high_layered_map->points[point_id].z=0;
            high_layered_map->points[point_id].intensity=3;
        }

        
    }
    pcl::toPCLPointCloud2(*low_layered_map, low_cloud);
    pcl::toPCLPointCloud2(*mid_layered_map, mid_cloud);
    pcl::toPCLPointCloud2(*high_layered_map, high_cloud);

    sensor_msgs::PointCloud2 low_output;
    sensor_msgs::PointCloud2 mid_output;
    sensor_msgs::PointCloud2 high_output;
    pcl_conversions::fromPCL(low_cloud,low_output);
    pcl_conversions::fromPCL(mid_cloud,mid_output);
    pcl_conversions::fromPCL(high_cloud,high_output);
    low_output.header.frame_id = "map";
    mid_output.header.frame_id = "map";
    high_output.header.frame_id = "map";
    low_pcd_pub.publish(low_output);
    mid_pcd_pub.publish(mid_output);
    high_pcd_pub.publish(high_output);
...
int main(int argc, char** argv)
{
  ...
  low_pcd_pub = nh.advertise<sensor_msgs::PointCloud2>("low_points_map", 1, true);
  mid_pcd_pub = nh.advertise<sensor_msgs::PointCloud2>("mid_points_map", 1, true);
  high_pcd_pub = nh.advertise<sensor_msgs::PointCloud2>("high_points_map", 1, true);
